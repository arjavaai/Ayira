...
const AnimatedTabs = ({
  ...
  return (
    <div className={cn("w-full max-w-4xl flex flex-col gap-y-1", className)}>
      <div className="flex gap-1 sm:gap-2 flex-wrap bg-gradient-to-r from-sky-400 to-blue-600 p-1 rounded-xl shadow-lg">
        {tabs.map((tab) => (
          <button
            key={tab.id}
          ...
          >
            {activeTab === tab.id && (
              <motion.div
                layoutId="active-tab"
                className="absolute inset-0 bg-white/20 backdrop-blur-sm !rounded-lg shadow-lg"
      ...
      <div className="p-2 sm:p-4 bg-gradient-to-r from-sky-400 to-blue-600 shadow-lg text-white rounded-xl border min-h-60 h-full">
        {tabs.map(
          (tab) =>
            activeTab === tab.id && (
              <motion.div
